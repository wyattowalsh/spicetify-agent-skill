...
The grouped local equivalents are:
...
Repository automation runs these gates in GitHub Actions:

- `CI` runs on pushes and pull requests to `main`.
- `Release Verify` runs on `v*` tag pushes and manual tag verification.

Both workflows keep real Spicetify disabled, activate the pinned `pnpm@11.5.2`
version through Corepack before docs dependency installation, and validate only fake fixtures,
local schemas, the installed skill payload, docs, OpenSpec, evals, and the
`spicetify-agent` helper.
