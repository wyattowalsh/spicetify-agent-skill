---
title: Schema notes
description: Migrated /spicetify planning content for Schema notes.
section: reference
source: migrated
sourcePath: docs/content/docs/archive/add-spicetify-skill/schemas/index.mdx
owner: docs
status: migrated
safetyLevel: read
---
...
**Path:** `docs/content/docs/archive/add-spicetify-skill/schemas/index.mdx`
...
All required schema files live under `skills/spicetify/assets/schemas/`; the
filenames below are relative to that skill-local source directory.
...
| Schema | Purpose |
|---|---|
| `skills/spicetify/assets/schemas/desired-state-manifest.schema.json` | Declarative manifest import/export and replay. |
| `skills/spicetify/assets/schemas/asset-manifest.schema.json` | Asset trust, ownership, file hashes, and install state. |
| `skills/spicetify/assets/schemas/redaction-policy.schema.json` | Report shareability and sensitive-value handling. |
| `skills/spicetify/assets/schemas/consent-grant.schema.json` | Bounded approval for logs, screenshots, launch flags, source archives, network staging, and build scripts. |

## Docs-site schemas

| Schema | Purpose |
|---|---|
| `skills/spicetify/assets/schemas/docs-site-manifest.schema.json` | Validates the planned Fumadocs app root, routes, content sources, UI policy, validation, and safety gates. |
| `skills/spicetify/assets/schemas/docs-page.schema.json` | Validates docs frontmatter and generated-page metadata. |

## Docs-site schemas

- `skills/spicetify/assets/schemas/docs-site-manifest.schema.json` — validates the docs-site package/source/content outputs planned for the Fumadocs site.
- `skills/spicetify/assets/schemas/docs-page.schema.json` — validates page metadata, source traceability, and safety labels.
- `skills/spicetify/assets/schemas/docs-site-config.schema.json` — validates planned Fumadocs + shadcn/ui site configuration, content sections, custom components, and validation gates.
- `skills/spicetify/assets/schemas/docs-site-content.schema.json` — validates documentation information architecture, required mode pages, workflow pages, schema pages, and redaction policy for examples.

## Subagent schemas

- `skills/spicetify/assets/schemas/subagent-task-graph.schema.json` validates the Codex swarm DAG, including agents, waves, merge protocol, and fallback.
- `skills/spicetify/assets/schemas/subagent-result.schema.json` validates each subagent result envelope before orchestrator consolidation.
