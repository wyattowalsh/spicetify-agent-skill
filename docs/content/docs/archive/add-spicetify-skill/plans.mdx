---
title: PLANS.md — ExecPlan for `/spicetify`
description: Migrated /spicetify planning content for PLANS.md — ExecPlan for `/spicetify`.
section: planning
source: migrated
sourcePath: docs/content/docs/archive/add-spicetify-skill/plans.mdx
owner: docs
status: migrated
safetyLevel: read
---
...
**Path:** `docs/content/docs/archive/add-spicetify-skill/plans.mdx`
...
6. `docs/content/docs/archive/add-spicetify-skill/source-refresh.mdx`
7. `docs/content/docs/archive/add-spicetify-skill/policy-matrix.mdx`
8. `docs/content/docs/archive/add-spicetify-skill/invariants.mdx`
9. `docs/content/docs/archive/add-spicetify-skill/failure-recovery-catalog.mdx`
10. `docs/content/docs/archive/add-spicetify-skill/operation-state-machine.mdx`
11. `docs/content/docs/archive/add-spicetify-skill/codex-handoff.mdx`
...
- The implementation is rooted at `<repo>`; the former `spicetify_skill_planning_bundle/` directory has been removed after root validation passed.
- `spicetify-agent` is the installable console command so it does not shadow the real `spicetify` binary.
- Python package metadata is in `pyproject.toml`; docs-site Node metadata is isolated under `docs/`.
...
- `setuptools` is the package build backend, allowing offline `uv build --offline` with the installed `uv` executable.
- Runtime dependencies remain empty; dev/test tools are isolated in the `dev` dependency group so `uv run pytest`, `uv run ruff`, and `uv run ty check src` are reproducible without adding runtime package authority.
...
- Package audit found `validate-schemas` could return success with zero schemas
  outside the repo checkout. Skill-local schemas under `skills/spicetify/assets/schemas/`
  are authoritative for repo work, and synchronized package data under
  `skills/spicetify/scripts/_schema_data.py` now supports installed `pip`/`uvx` runs.
...
| Docs package scripts | Keep docs dependency/build authority isolated to `docs` and the workspace lockfile. | The user approved docs dependency install/build work; tracked `onlyBuiltDependencies` allows required `esbuild`/`sharp` build scripts without granting the Python operator any Node runtime authority. | New docs dependencies, registry scaffolds, package scripts, or deploy/publish actions are requested. |
...
| `python3 tools/validate_bundle.py --root .` | passed | 2026-06-13: validator reported 248 files, 23 spec domains, 28 skill-local schemas, 22 mode docs, 14 workflows, 35 evals, no errors or warnings. |
...
| JSON schema parse | passed with `python3` | 2026-06-13: all `skills/spicetify/assets/schemas/*.json` parsed; output `schemas ok`. |
| `PYTHONPATH=skills/spicetify/scripts python3 -m pytest tests` | passed | 2026-06-13: direct host pytest run passed before dev dependency group addition; uv-managed pytest now passes 71 tests. |
...
| `uv run --frozen ty check src` | passed | 2026-06-16 docs-steward refresh: current type validation uses `ty`; earlier type-check evidence is superseded. |
| `PYTHONPATH=skills/spicetify/scripts python3 -m spicetify_agent --help` | passed | 2026-06-13: all planned mode commands and `execute-plan`/`validate-schemas` are exposed. |
| `PYTHONPATH=skills/spicetify/scripts python3 -m spicetify_agent validate-schemas` | passed | 2026-06-13: all bundled skill-local schemas loaded. |
...
| `node docs/scripts/validate-docs-content.mjs` | passed | 2026-06-13: strict docs validator reported 12 MDX pages ok. |
| `uv --version` | passed | 2026-06-13: `uv 0.11.17` is available. |
| `uv build --offline` | passed | 2026-06-13: built sdist and wheel with the configured build backend. |
...
| `python3 -m build --version` | not applicable | 2026-06-13: project now uses `setuptools`; `python -m build` remains optional external tooling and was not installed. |
...
| T6 docs-skill team | TASK-070..073 | `skills/spicetify/**`, `docs/**`, docs content | interrupted, then completed by orchestrator | `pnpm --filter docs validate:content` passed | Subagent did not return an envelope before interruption; generated dependency artifacts were removed. |
...
| T6 generated docs references | `tools/generate_docs_references.py`, `docs/content/docs/reference/**`, docs map | interrupted after timeout; completed by orchestrator | generated-reference check passed; docs validators passed | Partial docs edits were consolidated; duplicate stale `docs/scripts/generate-references.py` was removed. |
...
| T8 post-fix review | final blocker recheck | read-only | interrupted parent after timeout; child reviewers A8.1/A8.2 completed | A8.1 passed focused plan hash/policy/update tests; A8.2 passed focused runner/fake-binary tests; no critical/high findings in reviewed scope | Orchestrator also relied on direct post-fix validation evidence: 71 tests, ruff, type-checking, docs validators, generated-reference check, bundle validator, and approved OpenSpec validation passed. |
| docs-steward review | docs/API/skill/file-structure review | review only | attempted, interrupted after timeout | direct docs validators passed | `/docs-steward` was available as a skill source in `<redacted-path>`, but the review subagent did not return an envelope in time. |
...
| `uv run --frozen ruff format --check . && uv run --frozen ruff check . && uv run --frozen ty check src` | passed | 37 files formatted, lint clean, type check clean over 18 source files. |
...
| `skills/spicetify/assets/schemas/*.json` parse loop | passed | Printed `schemas ok`. |
...
| `uv run --frozen ruff format --check . && uv run --frozen ruff check . && uv run --frozen ty check src` | passed | 41 files formatted, lint clean, type check clean over 18 source files. |
...
## 2026-06-16 docs-steward route and inventory pass

- Ran docs-steward review lanes across installed skill references, planning/OpenSpec docs, and the Fumadocs companion app.
- Fixed stale eval wording so report/artifact assertions are documented as planned dry-run artifacts unless fake execution is explicitly enabled.
- Documented `suiteKind: canonical` and `suiteKind: focused` in eval, validation, and Codex handoff docs.
- Added a browsable `/docs/[[...slug]]` route for the MDX content tree and updated the docs manifest so `/docs` is an explicit route.
- Centralized docs page, mode, and schema inventory through `docs/content-map.json` and tightened content validation to fail on duplicate sections, missing pages, stale mode lists, stale schema lists, and missing docs routes.
- Updated `llms.txt` and `llms-full.txt` to derive from the same docs metadata and current mode inventory.
- Cleaned generated docs/build caches after validation; dependency directories were left intact.

Docs-steward review results:

| Agent | Status | Key findings | Resolution |
|---|---|---|---|
| `docs_skill_refs_review` | completed | Skill references were self-contained; adapter-list drift was noted but not applied because active repo install guidance does not require the unsupported `grok` adapter. | No skill payload change. |
| `docs_planning_openspec_review` | completed | Eval planning docs still used produced-artifact wording and did not document canonical/focused suite kinds. | Updated `eval-stack.md`, `validation.md`, and `codex-handoff.md`; OpenSpec testing spec now says unplanned artifacts. |
| `docs_fumadocs_review` | completed with findings | Docs build lacked `/docs` routes; content map omitted `evolve` and newer schemas; LLM routes were static and stale. | Added `/docs/[[...slug]]`, content helper, metadata-backed LLM routes, stricter content validation, and refreshed docs manifest/content map. |

Validation evidence from this pass:

| Command | Status | Notes |
|---|---|---|
| `python3 tools/generate_docs_references.py --root . --check` | passed | Generated reference pages fresh: 4 pages. |
| `pnpm --filter docs lint` | passed | Docs lint ok. |
| `pnpm --filter docs typecheck` | passed | TypeScript typecheck passed. |
| `pnpm --filter docs validate:content` | passed | 12 MDX pages validated, including exact mode/schema inventory checks. |
| `pnpm --filter docs build` | passed | Next build generated 17 static pages and included `/docs/[[...slug]]` with `/docs` and content routes. |
| `python3 tools/generate_docs_references.py --root .` | passed | 4 pages generated, 0 changed. |
| `python3 tools/validate_bundle.py --root . --write-manifest` | passed | Manifest regenerated; validator reported 308 files, 23 specs, 37 schemas, 23 mode docs, 14 workflow docs, 48 evals, no warnings. |
| `python3 tools/validate_bundle.py --root .` | passed | Same counts as the write-manifest pass; valid true. |
| `python3 tools/validate_openspec_structure.py --root .` | passed | 23 configured domains, 23 spec domains, 38 task IDs. |
| `python3 -m json.tool evals/regression-prompts.json >/dev/null` | passed | JSON parsed. |
| `skills/spicetify/assets/schemas/*.json` parse loop | passed | Printed `schemas ok`. |
| `npx --yes --package @fission-ai/openspec@1.4.1 openspec validate add-spicetify-skill --strict && npx --yes --package @fission-ai/openspec@1.4.1 openspec validate --all --strict` | passed | Active change and all changes validated strictly. |
| stale-term `rg` scan | passed | No stale `mypy`, `uv build --no-index`, produced-artifact, or removed placeholder module references remained in docs/code surfaces. |
