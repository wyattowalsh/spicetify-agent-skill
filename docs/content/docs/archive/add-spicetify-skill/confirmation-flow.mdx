...
description: Migrated /spicetify planning content for Confirmation flow.
section: planning
source: migrated
sourcePath: docs/content/docs/archive/add-spicetify-skill/confirmation-flow.mdx
owner: docs
status: migrated
safetyLevel: read
---
...
**Path:** `docs/content/docs/archive/add-spicetify-skill/confirmation-flow.mdx`
...
See `skills/spicetify/assets/schemas/confirmation.schema.json`. The implementation should persist confirmation grants beside operation records and reference them from `OperationRun` reports.
...
Confirmations approve mutations or high-risk plan effects. Consent grants approve bounded evidence collection such as logs, screenshots, launch flags, source archives, network staging, or build-script execution. A plan may need both. Consent grants are modeled separately in `skills/spicetify/assets/schemas/consent-grant.schema.json` so evidence scope, duration, output limit, redaction policy, and shareability are explicit.
