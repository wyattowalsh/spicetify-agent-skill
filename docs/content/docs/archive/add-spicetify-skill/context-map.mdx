---
title: Context map
description: Migrated /spicetify planning content for Context map.
section: planning
source: migrated
sourcePath: docs/content/docs/archive/add-spicetify-skill/context-map.mdx
owner: docs
status: migrated
safetyLevel: read
---
...
**Path:** `docs/content/docs/archive/add-spicetify-skill/context-map.mdx`
...
Checked on 2026-06-13 in `<repo>`.

| Evidence | Result | Notes |
|---|---|---|
| Git root | `<repo>` | `git rev-parse --show-toplevel` returned this path. |
| Implementation root | `<repo>` | The former `spicetify_skill_planning_bundle/` source directory was promoted into the repo root and removed after root validation passed. |
...
| Package manager | Python package uses `pyproject.toml` and `uv.lock`; docs app has isolated Node metadata under `docs/`, `pnpm-lock.yaml`, and an ignored local `node_modules/` install | User approved docs dependency install/build work on 2026-06-13. Runtime dependency set remains empty, and docs Node tooling has no authority inside the Python operator. |
| Runtime package | `skills/spicetify/scripts/` with console command `spicetify-agent` | `/spicetify` remains the skill name; no executable named `spicetify` is created. |
...
| Write scope for current implementation | skill-local runtime, tests, skill-local schemas under `skills/spicetify/assets/schemas/`, docs, OpenSpec, planning files, and package-manager lockfiles named by the approved implementation plan | Continue to avoid commits, pushes, deploys, permission changes, external service mutations, and additional dependency changes unless explicitly requested. |
...
| W1 Contracts | TASK-010..012 | `skills/spicetify/assets/schemas/`, command workflow, validation | `skills/spicetify/assets/schemas/`, tests, fixtures | schema parse results, fake binary behavior |
...
| TASK-073 | `skills/spicetify/assets/schemas/*.schema.json`, `modes/*.md`, `traceability.md`, `docs-content-architecture.md` | Generated docs must include source traceability and redaction checks. |
...
| A1 contracts-and-schemas | `api-contracts.md`, `skills/spicetify/assets/schemas/`, specs | schema drift and validation fixtures | skill-local schema tests only |
